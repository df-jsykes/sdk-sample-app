...
$(document).on("api:system:ready", function () {
    myApp.getSession = function () {
        ...
        progressBar.show();
        //check for Launchpad above
        if(window.top && window.top.CurrentSession){
            var response = window.top.CurrentSession;
    ...
    myApp.showData = function (table_name) {
        ...
        dreamfactory.db.getRecords({table_name: table_name, include_schema: true}).then(function (response) {
                ...
                data.forEach(function (record) {
                    ...
                    tables += "<td style='width:100px;'><button onclick=myApp.saveData(event,'" + table_name + "') class='btn btn-default'><span class='glyphicon glyphicon-floppy-disk'></span></button><button onclick=myApp.deleteData(event,'" + table_name + "') class='btn btn-default'><span class='glyphicon glyphicon-remove'></span></button></td>";
                    tables += "</tr>";
                });
                tableData.html(tables);
                tableContainer.show();
        ...
        myApp.insertData = function (table_name) {
            var formItems = $(".form-control.form-item");
            var record = {};
